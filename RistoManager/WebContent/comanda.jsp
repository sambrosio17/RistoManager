...
<body>
	<div class="main_container">
		...
		<div class="container ">
			<div class="category_container">
				...
				<h3>Codice Tavolo: <%=c.getCodiceTavolo() %></h3>
			</div>
			...
			<%if(comanda!=null){
				...
				<div class="single_product sp_comanda">
					<div class="sx">
						<img
							src="<%=p.getProdotto().getImmagine() %>" />
					</div>
					<div class="dx">
						<div class="title_in_comanda">
							<h3 onclick="showDetails(<%=p.getProdotto().getId()%>)"><%=p.getProdotto().getNomeprodotto() %></h3>
							<i class="fas fa-trash" onclick="removeItem(<%=p.getProdotto().getId()%>)"></i>
						</div>
						<p><%=p.getProdotto().getDescrizione() %></p>
						<div class="mini_form">
							<h3><%=p.getQuantita()*p.getProdotto().getPrezzo() %>&euro;</h3>
							<form action="./updateComanda" method="POST">
								<label for="quantita">Quantita: </label> <input type="number"
									name="quantita" min=1 default=1 value=<%=p.getQuantita() %>> <input type="text"
									value="<%=p.getProdotto().getId() %>" hidden="true" name="productId">
	...
	<div class="totale_container">
		<span>Totale: <%=comanda.getTotale() %>&euro;</span> <a href="./sendComanda">INVIA L'ORDINE</a>
	</div>
	<%} %>
